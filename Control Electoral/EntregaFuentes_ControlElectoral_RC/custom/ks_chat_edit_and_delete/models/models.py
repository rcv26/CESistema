...
class KsChatDelete(models.Model):
    ...
    #     action = self.env['ir.actions.act_window']._for_xml_id('ks_chat_edit_and_delete'
    #                                                            '.action_mail_message_extension_new')
    ...
    def _message_format(self, fnames):
        vals_list = super(KsChatDelete, self)._message_format(fnames)
        for vals in vals_list:
            message_sudo = self.browse(vals['id']).sudo().with_prefetch(self.ids)
            if message_sudo.ks_msg_edit == True:
                vals['ks_msg_edit'] = True
            else:
                vals['ks_msg_edit'] = False
        return vals_list
